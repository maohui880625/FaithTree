...
export default class home extends Component {
    ...
    _renderRefreshControl() {
        return (
            <RefreshControl
                refreshing={this.state.isRefreshing}//通过this.state.isRefreshing设置RefreshControl是否正在刷新
                onRefresh={this._onRefresh}//刷新时调用onRefresh()方法
                tintColor={'#ff0000'}
                title={'正在刷新数据，请稍后……'}
                titleColor={'#0000ff'}
            />
    ...
    _onRefresh = () => {
        this.setState({isRefreshing: true});//设置状态为正在刷新

        setTimeout(() => {
            const products = Array.from(new Array(10)).map((value, index) => ({
                image: require('./images/flag.jpg'),
                title: '新活动' + index,
                subTitle: '新活动描述' + index
            }));
            this.setState({isRefreshing: false, dataSource: ds.cloneWithRows(products)});//设置状态为结束刷新
        }, 2000);//定时器间隔2秒
    }
}
